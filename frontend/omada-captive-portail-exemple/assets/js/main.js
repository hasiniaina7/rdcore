...
var radiusMode = 'user';
var THEME_KEY = 'portalTheme';
var INFO_DASHBOARD_URL = 'http://167.86.71.186:5173/success?key=test_dynamic_keys';
...
function applyPortalTheme(mode) {
    ...
    if(mode === 'light'){
        ...
        toggle && (toggle.textContent = 'Mode sombre');
    }else{
        body.classList.remove('light-theme');
        toggle && (toggle.textContent = 'Mode clair');
    }
...
function initTermsModal(){
    var modal = document.getElementById('modalTerms');
    ...
    agreeBtn && agreeBtn.addEventListener('click', closeModal);
...
Ajax.post(
    ...
    function (res) {
        ...
        if(window.authType === EXTERNAL_RADIUS || window.authType === RADIUS_ACCESS_TYPE){
            syncRadiusVoucherPassword();
        }
        ...
        function hotspotChang (type) {
            ...
            document.getElementById("input-phone-num").style.display = "none";
            document.getElementById("input-verify-code").style.display = "none";
            document.getElementById("radius-mode").classList.add("hidden");
            resetRadiusMode();
            ...
            switch (Number(type)) {
                ...
                case EXTERNAL_LDAP:
                    ...
                    if(window.authType === EXTERNAL_RADIUS || window.authType === RADIUS_ACCESS_TYPE) {
                      document.getElementById("radius-mode").classList.remove("hidden");
                    }
                    break;
        ...
        setupRadiusModeListeners();
        pageConfigParse();
        setupThemeToggle();
    }
);

function resetRadiusMode() {
  radiusMode = 'user';
  var buttons = document.querySelectorAll('.radius-mode-btn');
  buttons.forEach(function (btn) {
    btn.classList.toggle('active', btn.dataset.mode === radiusMode);
  });
...
function setupRadiusModeListeners() {
  var container = document.getElementById('radius-mode');
  if (!container) return;
  var buttons = container.querySelectorAll('.radius-mode-btn');
  buttons.forEach(function (btn) {
    btn.addEventListener('click', function () {
      radiusMode = btn.dataset.mode;
      ...
      if (radiusMode === 'voucher') {
        ...
        passwordInput.value = document.getElementById('username').value;
      } else {
        passwordInput.readOnly = false;
      }
    });
  ...
  if (usernameInput) {
    usernameInput.addEventListener('input', function (event) {
      if (radiusMode === 'voucher') {
        document.getElementById('password').value = event.target.value;
      }
    });
  }
}

function syncRadiusVoucherPassword() {
  if (radiusMode !== 'voucher') return;
  var usernameField = document.getElementById('username');
  var passwordField = document.getElementById('password');
  if (usernameField && passwordField) {
    passwordField.value = usernameField.value;
    console.log('[omada-portal] voucher login', usernameField.value);
  }
}
...
function setupThemeToggle() {
  ...
  toggle.addEventListener('click', function () {
    ...
    toggle.textContent = document.body.classList.contains('light-mode') ? 'Mode sombre' : 'Mode clair';
  });
}
