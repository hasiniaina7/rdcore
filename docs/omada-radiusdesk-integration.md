...
| Détection session active | `/ip/hotspot/active/print`, `/ppp/active/print` via `MikrotikApiComponent` lignes 17‑108. | API publique absente → besoin d’un service abstraction (scraper `extPortal/*` ou utiliser SNMP?). | Nécessite un “OmadaSessionsStore” dans notre backend pour corréler `authCode` ⇔ `username`, puisque l’API Omada ne liste pas les sessions. |
| Kick / CoA | `KickerComponent` + `MikrotikApiComponent::kickRadius` (API RouterOS) et fallback `radclient` PoD. | Pas d’API CoA officielle. Options: `extPortal/disconnect` (disponible à partir v5.13) ou appel `clients/setAccess`. Sinon fallback RADIUS `Disconnect-Request`. | Implémentation à écrire : `OmadaCoaService` qui rejoue `extPortal/auth` avec `authCode` = block, ou `clients/unauthorize`. |
| Profils / quotas | RouterOS gère les limites; RD pousse `Rd-*` attributs dans Access‑Accept. | Omada attend un `accessToken` (opaque) et applique les ACL en interne; quotas doivent rester côté RD (radacct). | Le mapping attributs doit être conservé côté RADIUS; Omada n’expose pas de profil. Utiliser `accessToken=username` pour conserver la traçabilité. |
