...
- **Backend Node** :
  - La variable d’environnement `OMADA_EXTERNAL_PORTAL_ENABLED` (voir `apps/backend-portal-api-rd-omada/src/config.ts`) permet d’activer/désactiver l’appel `extPortal/auth` côté backend.
