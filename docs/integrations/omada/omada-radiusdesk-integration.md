...
| Services Node existants     | `apps/backend-portal-api-rd-omada/src/services/radiusdeskIntegration.ts`, `usageService.ts`, `authService.ts`, `omadaIntegration.ts`                    | `radiusdeskIntegration` proxy `/dynamic-details`, `/permanent-users`, `/radaccts` pour la React app et normalise la télémétrie. `usageService` fusionne `/radaccts/get-usage` et `/radaccts/index`. `authService` vérifie les identifiants RD (permanent/voucher), génère un `requestId`, puis délègue à `omadaIntegration.authorizeClient`.                              |
...
- **Mode Local Web Portal (page interne/importée)** :
  ...
  - La personnalisation se fait via “Import Customized Page” à partir d’un bundle statique, désormais maintenu dans `[legacy removed from repository]`. Ce bundle reproduit une Dynamic Login Page RadiusDesk (Bootstrap 5, i18n, bloc usage) tout en conservant le flux RADIUS natif Omada.
...
#### 3.0.1 Bundle `[legacy removed from repository]`

- **Entrée / fichiers clés** :
  - `index.html` : copie modernisée du portail TP-Link officiel (`[legacy removed from repository]`). Aucun appel RadiusDesk n’est effectué ; la page consomme `/portal/getPortalPageSetting`, gère les différents modes Hotspot (voucher, SMS, local user, etc.) et poste vers `/portal/radius/auth` comme le ferait Omada par défaut.
  - `assets/js/main.js` : script original TP-Link, juste enrichi d’un switch “Utilisateur / Voucher” pour les modes `authType = 2` ou `8` (username = password) et d’un bouton de thème clair/sombre.
  - `assets/js/jquery.min.js`, `assets/css/style.css`, `assets/img/*` : assets statiques issus de la démo Omada.
  - `package.sh` : génère `legacy-omada-bundle.zip` (bundle importable dans l’UI Omada).
- **Flux** :
  1. L’appareil est redirigé vers `index.html?...` avec tous les paramètres Omada. `main.js` affiche automatiquement le formulaire adapté selon `authType`.
  2. Pour `authType = 2/8`, l’opérateur peut choisir “Utilisateur” ou “Voucher” : en mode Voucher, le mot de passe prend la même valeur que l’identifiant (comme demandé) et le formulaire est posté vers `/portal/radius/auth`.
  3. Les codes d’erreur Radius (`errorCode`) sont relayés tels quels par `main.js` (affichage dans `#oper-hint`), ce qui permet de voir les messages FreeRADIUS/Omada originaux (“Invalid authentication type”, “Incorrect password”, etc.).
- **Pré-requis infra** :
  ...
  - Aucun appel AJAX externe n’est effectué avant auth (tous les fetch restent internes à Omada), donc pas de configuration CORS particulière.
- **Validation rapide** :
  - Localement : `bundle local non versionné (retiré du repo)` puis `http://localhost:4173/index.html?clientMac=AA...`.
  - En prod : importer `legacy-omada-bundle.zip`, associer au SSID, puis vérifier que les erreurs/acceptations Radius remontent correctement dans `#oper-hint`.
...
- **Portée** :
  ...
  - **Bundle livré** : `[legacy removed from repository]` (HTML/JS/CSS prêt à zipper via `zip -r legacy-omada-bundle.zip .`) qui consomme `dynamic-details/info-for.json` / `radaccts/get-usage.json` et poste les identifiants vers `/portal/radius/browserauth`.
...
> **Remarque** : le dossier `[legacy removed from repository]` (lorsqu’il est présent dans l’arborescence) représente un exemple de portail local compatible Omada. Il doit être conservé comme référence technique/UX pour le mode Local Web Portal et ne doit pas être cassé par les évolutions du projet `dev/captive-portail`.
...
**Contexte**  
Omada est configuré en Hotspot avec `Authentication Type = RADIUS Server` et `Portal Customization = Local Web Portal` (page interne ou importée). Le bundle `[legacy removed from repository]` sert de base de portail local compatible Omada (voucher, local user, RADIUS, form auth…).

**Objectif**  
Consolider ce mode Local Web Portal en :
- préservant `[legacy removed from repository]`,
...
- `[legacy removed from repository]` (structure, README, assets).  
...
1. Auditer le bundle `[legacy removed from repository]` (types d’auth supportés, paramètres attendus, hooks Omada).  
...
4. Ajouter, si nécessaire, un guide succinct (README) dans `[legacy removed from repository]` expliquant comment builder et importer le bundle dans Omada.  

**Contraintes**  
- Ne pas casser le bundle existant : pas de refacto profond ni de dépendance directe avec le monorepo `apps/frontend-portal-web-personalized/`.  
...
- Un opérateur peut :  
  - builder/importer le bundle `legacy omada bundle` dans Omada,  
...
**Périmètre & fichiers concernés**  
- Backend :  
  - `apps/backend-portal-api-rd-omada/src/config.ts` (flag de mode External).  
  - `apps/backend-portal-api-rd-omada/src/services/omadaIntegration.ts` (client HTTP Omada).  
  - `apps/backend-portal-api-rd-omada/src/services/authService.ts` (orchestrateur connect + extPortal/auth).  
  - `apps/backend-portal-api-rd-omada/src/routes/connect.ts` (endpoint `/connect/:mode`).  
- Frontend :  
  - `apps/frontend-portal-web-personalized/src/hooks/useOmadaParams.ts`.  
  - `apps/frontend-portal-web-personalized/src/modules/dynamic/connectUtils.ts`.  
  - `apps/frontend-portal-web-personalized/src/modules/dynamic/ConnectPanel.tsx`.  
  - `apps/frontend-portal-web-personalized/src/pages/Home.tsx`.  
- Documentation :  
...
**Étapes d’implémentation**  
1. **Configuration** :  
   - Ajouter un flag `OMADA_EXTERNAL_PORTAL_ENABLED` dans `apps/backend-portal-api-rd-omada/src/config.ts` (déjà présent si ce prompt a été appliqué une première fois), par défaut `true`.  
...
- Backend Node :  
  - `apps/backend-portal-api-rd-omada/src/services/radiusdeskIntegration.ts` (`kickSessions`).  
...
**Périmètre & fichiers concernés**  
- Backend :  
  - tests unitaires/integration (`apps/backend-portal-api-rd-omada/tests` ou équivalent).  
  - instrumentation metrics/logs (`apps/backend-portal-api-rd-omada/src/utils/metrics.ts`, logger).  
- Frontend :  
