...
| `apps/backend-portal-api-rd-omada/.env` | Secrets backend (tokens RadiusDesk, accès Omada, identifiants admin statiques). | Ne jamais pousser sur Git; synchroniser via gestionnaire sécurisé. |
| `apps/frontend-portal-web-personalized/.env` | Variables spécifiques au frontend (URL API, clés SSO publiques). | Utiliser uniquement pour le build local; pour la CI/CD passer par des variables chiffrées. |
